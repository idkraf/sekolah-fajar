...
class Kategori_set extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('Kategori_model', 'admin');
    ...
    if($getId == NULL){
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Kategori";
            $data['kategori'] = $this->admin->get('kategori', ['idkategori' => $id]);
            ...
            $input = $this->input->post(null, true);
            $update = $this->admin->update('kategori', 'idkategori', $id, $input);
            if ($update) {
                $this->session->set_flashdata('success','Data berhasil disimpan');
                redirect('manage/kategori');
            } else {
    ...
    public function delete()
    {
        ...
        if ($this->admin->delete('kategori', 'idkategori', $id)) {
    ...
    public function ajax_list() {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = $prd->keterangan;
            $row[] = '<a href="'.base_url('manage/kategori/edit'). '/' . $prd->idkategori .'"
            class="btn btn-warning btn-circle btn-sm"><i class="fa fa-edit"></i></a>
            <button data-object-id="'.$prd->idkategori.'"class="btn btn-success btn-circle btn-sm delete-object"><i class="fa fa-minus"></i></button>
            ';
            $data[] = $row;
        }
        $output = array(
