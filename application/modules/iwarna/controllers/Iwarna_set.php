...
class Iwarna_set extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('Iwarna_model', 'admin');
    ...
    if($getId == NULL){
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = "Warna";
            $data['warna'] = $this->admin->get('iwarna', ['id_warna' => $id]);
            $data['main'] = 'iwarna/edit';
            $this->load->view('manage/layout', $data);
            //$this->template->load('templates/dashboard', 'warna/edit', $data);
        } else {
            $input = $this->input->post(null, true);
            $update = $this->admin->update('iwarna', 'id_warna', $id, $input);
            if ($update) {
                $this->session->set_flashdata('success','Data berhasil disimpan');
                redirect('manage/iwarna');
            } else {
    ...
    {
        $id = encode_php_tags($getId);
        if ($this->admin->delete('iwarna', 'id_warna', $id)) {
