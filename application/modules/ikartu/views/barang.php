<div class="content-wrapper">
    ...
    <?php if (!empty($id_barang)) {
      $barang = $this->db->get_where('inventori', ['id_barang' => $id_barang])->row_array();
      $rasa = $this->db->get_where('irasa', ['id_rasa' => $barang['rasa_id']])->row_array();
      $jenis = $this->db->get_where('ijenis', ['id_jenis' => $barang['jenis_id']])->row_array();
      $merek = $this->db->get_where('imerek', ['id_merek' => $barang['merek_id']])->row_array();
      $stok_awal = $barang['stok_awal'];
      $masuk_min = $this->db->select('tanggal_masuk')->order_by('tanggal_masuk', 'asc')->limit(1)->get_where('inventori_masuk', ['barang_id' => $id_barang])->row_array();
      $masuk_max = $this->db->select('tanggal_masuk')->order_by('tanggal_masuk', 'desc')->limit(1)->get_where('inventori_masuk', ['barang_id' => $id_barang])->row_array();
      $keluar_min = $this->db->select('tanggal_keluar')->order_by('tanggal_keluar', 'asc')->limit(1)->get_where('inventori_keluar', ['barang_id' => $id_barang])->row_array();
      $keluar_max = $this->db->select('tanggal_keluar')->order_by('tanggal_keluar', 'desc')->limit(1)->get_where('inventori_keluar', ['barang_id' => $id_barang])->row_array();
              ...
              <table class="table data w-100">
                ...
                <tbody>
                  ...
                  foreach ($period as $dt) {
                    ...
                    $masuk = $this->db->query("SELECT SUM(`jumlah_masuk`) as `masuk` FROM `inventori_masuk` WHERE `barang_id` = '$id_barang' AND `tanggal_masuk` = '$tanggal'")->row_array()['masuk'];
                    $keluar = $this->db->query("SELECT SUM(`jumlah_keluar`) as `keluar` FROM `inventori_keluar` WHERE `barang_id` = '$id_barang' AND `tanggal_keluar` = '$tanggal'")->row_array()['keluar'];
