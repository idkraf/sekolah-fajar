...
class Obat_set extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('Obat_model', 'admin');
    ...
    {
        $this->form_validation->set_rules('judul', 'Keterangan', 'required|trim');
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $insert = $this->admin->insert('obat', $input);
            if ($insert) {
                $this->session->set_flashdata('success','Data berhasil disimpan');
                redirect('manage/obat');
            } else {
                $this->session->set_flashdata('failed','data gagal disimpan');
                redirect('manage/obat');
            }
    ...
    public function edit()
    {
        ...
        if ($id) {  
            $input = $this->input->post(null, true);
            $update = $this->admin->update('obat', 'id', $id, $input);
            if ($update) {
                $this->session->set_flashdata('success','Data berhasil disimpan');
                redirect('manage/obat');
            } else {
    ...
    public function delete()
    {
        ...
        if ($this->db->delete('obat', array('id' => $id))) {
    ...
    public function ajax_list() {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = $prd->unit;
            $row[] = $prd->dosis;
            $row[] = $prd->nama_obat; //nama buku
            $row[] = '<button            
            data-id="'.$prd->id.'" 
            data-unit="'.$prd->unit.'"
            data-judul="'.$prd->judul.'"
            data-dosis="'.$prd->dosis.'"
            data-nama-obat="'.$prd->nama_obat.'"
