...
class Inventoriout_set extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('Inventoriout_model', 'admin');
    ...
    private function _validasi()
    {
        ...
        $stok = $this->admin->get('inventori', ['id_barang' => $input]);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $kode = 'T-BK-' . date('ymd');
            $kode_terakhir = $this->admin->getMax('inventori_keluar', 'id_barang_keluar', $kode);
            $kode_tambah = substr($kode_terakhir, -5, 5);
            $kode_tambah++;
            $number = str_pad($kode_tambah, 5, '0', STR_PAD_LEFT);
            $data['id_barang_keluar'] = $kode . $number;
            $data['lokasi'] = "lokasi";   
            $data['main'] = 'inventoriout/add';
            $this->load->view('manage/layout', $data);
            //$this->template->load('templates/dashboard', 'barang_keluar/add', $data);
        } else {
            $input = $this->input->post(null, true);
            $insert = $this->admin->insert('inventori_keluar', $input);

            if ($insert) {
                //set_pesan('data berhasil disimpan.');
                redirect('manage/inventoriout');
            } else {
                //set_pesan('Opps ada kesalahan!');
                redirect('manage/inventoriout/add');
            }
        }
    }
    ...
    public function delete($getId)
    {
        ...
        if ($this->admin->delete('inventori_keluar', 'id_barang_keluar', $id)) {
    ...
    public function ajax_list() {
        ...
        foreach ($list as $prd) {
            ...
            $row[] = '<a onclick="return confirm("Yakin ingin hapus?")" href="'. base_url('manage/inventoriout/delete') . '/' . $prd->id_barang_keluar .'" class="btn btn-danger btn-circle btn-sm"><i class="fa fa-trash"></i></a>';
            $data[] = $row;
        }
        $output = array(
